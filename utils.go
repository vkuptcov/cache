...
func newContainer(dst interface{}) (containerInt, error) {
	...
	case reflect.Map:
		...
		if keyType.Kind() != reflect.String {
			return nil, fmt.Errorf("dst key type must be a string, %v given", keyType.Kind())
		}
	...
	default:
		return nil, fmt.Errorf("dst must be a map or a slice instead of %v", reflectValue.Type())
	}
